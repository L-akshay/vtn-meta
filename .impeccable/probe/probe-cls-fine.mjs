...
await page.addInitScript(() => {
  window.__ls = [];
  new PerformanceObserver(list => list.getEntries().forEach(e => { if (!e.hadRecentInput && e.value > 0.003) window.__ls.push({ t: Math.round(e.startTime), v: +e.value.toFixed(3), y: Math.round(scrollY), src: e.sources.map(s => { const n = s.node; return (n && (n.className?.baseVal ?? n.className) || n?.nodeName || '?') + ' ' + JSON.stringify([s.previousRect.y, s.currentRect.y]); }).slice(0, 3) }); })).observe({ type: 'layout-shift', buffered: true });
});
